"""
evaluate_caw.py — Evaluation for CropAndWeed dataset
Usage:
    python scripts/evaluate_caw.py \
...
def main():
    ...
    logger.info(f"Loaded: epoch={ckpt.get('epoch','?')}, best_mIoU={ckpt.get('best_miou',0):.4f}")
    ...
    metrics.print_summary(class_names=ds_cfg.get("class_names", ["background","crop","weed"]))
    r = metrics.compute()
